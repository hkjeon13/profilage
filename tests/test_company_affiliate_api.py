...
def test_profile_page_serves_company_profile_frontend():
    ...
    assert "/styles.css?v=company-profile-62" in response.text
    ...
    assert "/profile-page-5.js?v=company-profile-56" in response.text
...
def test_profile_overview_groups_company_information_without_relationship_card():
    ...
    assert "FSS 고유번호" not in basic_card_section
    assert "단축코드" not in basic_card_section
...
def test_profile_frontend_can_add_company_to_compare_list():
    ...
    assert "/profile-page-5.js?v=company-profile-56" in profile_response.text
...
def test_profile_mobile_layout_keeps_summary_near_first_viewport():
    with TestClient(app) as client:
        style_response = client.get("/styles.css")
...
def test_profile_frontend_does_not_duplicate_recent_disclosures():
    ...
    assert script_response.status_code == 200
    assert "renderDartDisclosures(info.dart_disclosures)" not in script_response.text
    assert "<h3>최근 공시</h3>" not in script_response.text
    assert "<h3>공시 이벤트</h3>" in script_response.text
    assert "renderDisclosureEventTimeline(info.disclosure_events)" in script_response.text
...
def test_profile_disclosures_open_inside_page_viewer():
    ...
    assert 'scrolling="yes"' in script_response.text
    assert 'class="disclosure-viewer-close" data-disclosure-viewer-close aria-label="닫기">&times;</button>' in script_response.text
...
def test_financial_summary_cards_open_trend_modal_with_account_checks():
    ...
    assert 'class="financial-trend-close" data-financial-trend-close aria-label="닫기">&times;</button>' in script_response.text
...
def test_financial_summary_and_disclosures_share_horizontal_row():
    ...
    row_renderer = script_response.text.split("function renderCompanyInsightRow(info)", 1)[1].split("function renderInsightMetric", 1)[0]
    assert "renderDartFinancialAccounts(info)" in row_renderer
    assert "renderDisclosureEventTimeline(info.disclosure_events)" in row_renderer
    assert "${financialSummary}" in row_renderer
    assert "${disclosureEvents}" in row_renderer
...
def test_profile_uses_disclosure_event_timeline_without_recent_disclosure_card():
    with TestClient(app) as client:
        script_response = client.get("/profile-page-5.js")

    assert script_response.status_code == 200
    assert "renderDartDisclosures" not in script_response.text
    assert "renderDisclosureEventTimeline" in script_response.text
    assert "${renderDisclosureEventTimeline(info.disclosure_events)}" not in script_response.text
    assert "<h3>최근 공시</h3>" not in script_response.text
    assert "<h3>공시 이벤트</h3>" in script_response.text
...
def test_relationship_summary_cards_open_company_list_modal():
    ...
    assert "/styles.css?v=company-profile-62" in profile_response.text
    assert "/profile-page-5.js?v=company-profile-56" in profile_response.text
...
def test_profile_frontend_exposes_disclosure_events_and_risk_signals():
    ...
    assert "text-overflow: ellipsis;" in style_response.text
    assert "width: fit-content;" in style_response.text
...
def test_disclosure_summary_loading_uses_pastel_shimmer_background_only():
    ...
    assert script_response.status_code == 200
    assert style_response.status_code == 200
    assert "disclosure-summary-skeleton" not in script_response.text
    assert "disclosure-summary-skeleton-line" not in script_response.text
    assert "disclosure-summary-skeleton-pill" not in script_response.text
    assert "disclosure-summary-loading-icon" not in script_response.text
    assert "linear-gradient(110deg" in style_response.text
    assert "animation: disclosure-summary-surface-shimmer" in style_response.text
...
async def test_company_info_service_attaches_normalized_dart_insights(monkeypatch):
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if path.endswith("/list.json"):
            assert request.url.params["page_count"] == "8"
            return httpx.Response(
                200,
                json={
                    "status": "000",
